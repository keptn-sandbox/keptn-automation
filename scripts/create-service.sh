...
DEBUG=${DEBUG:="false"}
CONFIGURE_DT_PROVIDER=${CONFIGURE_DT_PROVIDER:="false"}
...
# if upstream repo wanted, then ensure all inputs
if [ ! -z "$GIT_USER" ]; then
  GIT_TOKEN=${GIT_TOKEN:?'GIT_TOKEN envionmment variable missing'}
  GIT_REMOTE_URL=${GIT_REMOTE_URL:?'GIT_REMOTE_URL envionmment variable missing'}
fi
...
echo "-----------------------------------------------------------------"
echo "Checking if $KEPTN_PROJECT is already created"
if [ $(keptn get project $KEPTN_PROJECT | grep $KEPTN_PROJECT | wc -l) -ne 1 ]; then
  ...
  if [ ! -z "$GIT_USER" ]; then
    ...
    keptn create project $KEPTN_PROJECT --shipyard=$KEPTN_SHIPYARD_FILE --git-user=$GIT_USER --git-token=$GIT_TOKEN --git-remote-url=$GIT_REMOTE_URL
  fi
else
  echo "Project $KEPTN_PROJECT already created. Skipping create project"
fi

echo "-----------------------------------------------------------------"
echo "Checking if $KEPTN_SERVICE is already created"
if [ $(keptn get service $KEPTN_SERVICE --project $KEPTN_PROJECT | grep $KEPTN_SERVICE | wc -l) -ne 1 ]; then
  ...
  echo "Service $KEPTN_SERVICE already created. Skipping create service"
fi
...
echo "-----------------------------------------------------------------"
if [ "$CONFIGURE_DT_PROVIDER" == "true" ]; then
  echo "Configure Dynatrace provider is TRUE"
  ...
  echo "Skipping Configure Dynatrace provider"
fi
